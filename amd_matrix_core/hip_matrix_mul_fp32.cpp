...
// device function to handle matrix multiplication with size: TILE_SIZE_M, 
// TILE_SIZE_N, and TILE_SIZE_K (32, 32, 32)
// This kernel uses the mfma instruction 4X4X1_16fp32. The input tile size 
// 32 x BLOCK_SIZE_K and BLOCK_SIZE_K * 32, so each instruction needs
// BLOCK_SIZE_K / 1 interations. Bock_size is (16, 16), so 4 waves in a workgroup
__device__ void sgemm_fp32_4x4x1_fp32_device(const float *sa, const float *sb, float *bd, 
                                               const int lda, const int ldb, const int ldd, 
                                               const int stride_a, const int stride_b, const int stride_c, 
                                               const int K) {
    const int mfma_m = 4;
...
__global__ void sgemm_32x32xK_tile_v4(const float *A, const float *B, float *D, int M, int K, int N) {
    extern __shared__ float shared_mem[];
    const int slda = TILE_SIZE_K + 1;
    const int sldb = TILE_SIZE_N + 1;
    const int sldc = sldb;
    ...
    for (int tile_idx = 0; tile_idx < K; tile_idx += TILE_SIZE_K) {
        ...
        __syncthreads();
        sgemm_fp32_4x4x1_fp32_device(in_shared_a, in_shared_b, in_shared_c, 
                                       slda, sldb, sldc, 
                                       stride_a, stride_b, stride_c, TILE_SIZE_K);
        __syncthreads();
    }
...
bool hip_matrix_mul_sgemm_32x32xK_fp32_v4(CMatrix<float> &in1, CMatrix<float> &in2, CMatrix<float> &res, double& flops) {
    ...
    dim3 grid_dim_v2 = dim3((row + TILE_SIZE_M - 1) / TILE_SIZE_M, (column + TILE_SIZE_N - 1) / TILE_SIZE_N);
    dim3 block_dim_v2 = dim3(block_size_m, block_size_n);
    return run_kernel(sgemm_32x32xK_tile_v4, grid_dim_v2, block_dim_v2, lds_size, in1, in2, res, flops);
}
