...
int main(int argc, char **argv) {
    ...
    CMatrix<int8_t> matrix1(m, k), matrix2(k, n);
    matrix2.set_row_major(false);
    CMatrix<int32_t> res_matrix1, res_matrix2;
    ...
    if (ret == false) {
        cout << "int8 matrix dimension is incorrect, cannot multiplication." << endl;
        return 1;
    }
    timer.stop();
    ...
    if (ret == false) {
        cout << "int8 naive impl failed!" << endl;
        return 1;
    }
    timer.stop();
    size_t kernel_time_us = timer.gettime_us(); 
    cout << "hip_matrix_mul_int8_naive implementation = " << kernel_time_us << " us, flops = " << flops << " TFLOPS";
    ret = (res_matrix1 == res_matrix2);
    if (ret == false) {
        cout << "hip_matrix_mul_int8_naive implementation failed." << endl;
        return 1;
    }
    else {
        std::cout << ", PASSED!" << std::endl;
