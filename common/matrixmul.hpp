...
public:
    ...
    bool multiply_parallel(const CMatrix<T> &matrix, size_t thread_num, CMatrix<T> &res_matrix);
...
private:
    ...
    void multiply_thread(const CMatrix<T> &matrix, size_t thread_idx, size_t thread_num, CMatrix<T> &res_matrix);
...
bool CMatrix<T>::multiply_parallel(const CMatrix<T> &matrix, size_t thread_num, CMatrix<T> &res_matrix) {
    ...
    for (thread_idx = 0; thread_idx < thread_num; thread_idx++) {
        vec_tid[thread_idx] = thread(&CMatrix<T>::multiply_thread, this, matrix, thread_idx, thread_num, std::ref(res_matrix));
    }
