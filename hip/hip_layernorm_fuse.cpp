...
__global__ void layernorm_half2(void* in, void* w, void* b, float* m, float *v, void* data_out, int batch_item_num, int block_size)
{
    ...
    layernorm_kernel_half2(in_data, in_data_reduce, ww, bb, m, v, output, batch_item_num, block_size, rnum);
}
                               ...
                               int shared_size)
{
    ...
    int half2_block_size = block_size / 4;
    layernorm_half2<<<block_num, half2_block_size, shared_size>>>(
        in_d, w_d, bias_d, mean_d, var_d, out_d, batch_size, half2_block_size);
