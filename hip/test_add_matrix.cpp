...
// __global__ void kernel_add_matrix(float *a, float *b, float *c, int m, int n)
// {
//     extern __shared__ float lds[];

//     float *sa = lds;
//     float *sb = lds + BLOCK_M * BLOCK_N;
...
//     for (int i = 0; i < BLOCK_M; ++i) {
//         for (int j = tid; j < BLOCK_N; j += blockDim.y) {
//             float sum = sa[i * BLOCK_N + j] + sb[i * BLOCK_N + j];
...
__global__ void kernel_add_matrix1(float *a, float *b, float *c, int m, int n)
{
    extern __shared__ float lds[];

    float *sa = lds;
    float *sb = lds + BLOCK_M * (BLOCK_N + PADDING);
    int stride_n = (BLOCK_N + PADDING);
    ...
    int tid = threadIdx.y;
    for (int i = tid; i < BLOCK_M; i += blockDim.y) {
        for (int j = 0; j < BLOCK_N; ++j) {
            sa[i * stride_n + j] = a[(blk_i + i) * n + blk_j + j];
            sb[i * stride_n + j] = b[(blk_i + i) * n + blk_j + j];
        }
    }
    __syncthreads();
    ...
    for (int i = 0; i < BLOCK_M; ++i) {
        for (int j = tid; j < BLOCK_N; j += blockDim.y) {
            float sum = sa[i * stride_n + j] + sb[i * stride_n + j];
            c[(blk_i + i) * n + blk_j + j] = sum;
        }
    }
...
bool hip_add_matrix(const std::vector<T>& in1, const std::vector<T>& in2, std::vector<T>& res, int m, int n) {
    ...
    const std::size_t lds_size = 2 * sizeof(T) * BLOCK_M * (BLOCK_N + PADDING);
    // warm up to run the kernel for 10 times
    // for (int i = 0; i < 10; ++i) {
        hipLaunchKernelGGL(kernel_add_matrix1, dim3(m / BLOCK_M, n / BLOCK_N),
                            dim3(1, BLOCK_N), lds_size, 0,
